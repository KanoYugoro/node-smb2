...
class SmbReadableStream extends Readable {
  ...
  async _read (size) {
    ...
    while (this._queryReadIndex < this._queryLength || this._queryFinishedIndex < this._queryLength) {
      ...
      if (this._queryResultBuffer[this._queryFinishedIndex]) {
        ...
        const res = this.push(this._queryResultBuffer[this._queryFinishedIndex]);
        console.log(`Result: ${true}`);
        if (!res) {
          return
        }
        console.log(`Succeeded pushing ${this._queryFinishedIndex}`);

        this._queryResultBuffer[this._queryFinishedIndex] = null;
        this._queryFinishedIndex = this._queryFinishedIndex + 1;
      }

      console.log(`${this._activeQueries} ${this._maxQueries}`);
